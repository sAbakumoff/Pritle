...
e.foo(); //writes "e"
```
The disadvantage is in it's limitations : 
...
* Lack of design-time foolproof. f.e. if the line of code ```E.prototype = Object.create(C.prototype)``` was not added, e.foo() can still be invoked, but provides the error at runtime.
